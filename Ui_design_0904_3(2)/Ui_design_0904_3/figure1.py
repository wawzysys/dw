...
def figure1():
    file_path = r'C:\Users\19160\Desktop\dw\Loadshedding.xlsx'
    # 读取Excel文件
    ...
    # 绘制曲面图
    surf = ax.plot_surface(x_new,
                           y_new,
                           z_new,
                           cmap='winter',
                           edgecolor='none')  # 使用'winter'颜色图

    # 添加颜色条
    cbar = fig.colorbar(surf, ax=ax, shrink=0.5, aspect=5)
    cbar.set_label('负荷调控结果 (MWh)')
    ...
    # 保存图形
    plt.savefig('Figure1.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.show()
